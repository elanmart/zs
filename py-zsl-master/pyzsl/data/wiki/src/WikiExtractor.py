# THIS FILE IS A MODIFIED VERSION OF WIKIEXTRACTOR
# ORIGINAL CODE CAN BE FOUND AT
# BELOW IS THE LICENSE ASSOCIATED WITH THE ORIGINAL

# SEE COMMIT 13b44944e7e13fbc60cbd488a62f8e3d6097e5c4 FOR CHANGES THAT WERE MADE
...
import spacy
from pyzsl.data.wiki.src.dict_keys import EntryKeys
...
#
# Keys to split sections and detect categories
SECTION  = '<SECTION>'
CATEGORY = 'Category:'
...
def TOKENIZE(text):
    ...
    if options.lemmatize:
        return ' '.join([tok.lemma_ for tok in text])
    else:
        return ' '.join([tok.text   for tok in text])
    ...
    def fixup(m):
        ...
        try:
            if text_[1] == "#":  # character reference
                if text_[2] == "x":
                    return chr(int(code_[1:], 16))
                else:
                    return chr(int(code_))
            else:  # named entity
                return chr(name2codepoint[code_])
        except:
...
class Template(list):
    ...
    def parse(cls, body):
        ...
        #
        start = 0
        for s_, e_ in findMatchingBraces(body, 3):
            tpl.append(TemplateText(body[start:s_]))
            tpl.append(TemplateArg(body[s_ + 3:e_ - 3]))
            start = e_
...
class Extractor(object):
    ...
    def __init__(self, id_, revid, title, lines):
        """
    ...
    def write_output(self, out, text):
        ...
        n_letters = len(CATEGORY)

        for item in text:
            if not item.startswith(CATEGORY):
                text_.append(item)
            else:
                categories_.append(item[n_letters:])

        text = ' '.join(text_).replace('\n', ' ')

        if text.startswith(self.title):
            text = text[len(self.title):]

        (summary,
         *rest) = text.split(SECTION, 1)
        rest = ''.join(rest).replace(SECTION, ' ')
        ...
        if options.write_json:
            json_data = {
                EntryKeys.id:         self.id,
                EntryKeys.url:        url,
                EntryKeys.title:      self.title,
                EntryKeys.summary:    summary,
                EntryKeys.rest:       rest,
                EntryKeys.categories: categories_
            }
    ...
    def expand(self, wikitext):
        ...
        cur = 0
        # look for matching {{...}}
        for s_, e_ in findMatchingBraces(wikitext, 2):
            res += wikitext[cur:s_] + self.expandTemplate(wikitext[s_ + 2:e_ - 2])
            cur = e_
...
def splitParts(paramsList):
    ...
    for s_, e_ in findMatchingBraces(paramsList):
        par = paramsList[cur:s_].split(sep)
        if par:
            if parameters:
...
def sharp_if(extr, testValue, valueIfTrue, valueIfFalse=None, *_):
...
def sharp_ifeq(extr, lvalue, rvalue, valueIfTrue, valueIfFalse=None, *_):
    rvalue = rvalue.strip()
    if rvalue:
...
def sharp_iferror(extr, test, then='', Else=None, *_):
...
def sharp_invoke(module_, fn, args):
    functions = modules.get(module_)
    if functions:
        funct = functions.get(fn)
        if funct:
...
def callParserFunction(functionName, args, extractor):
    ...
    try:
        ...
        if functionName == '#invoke':
            mod, fun = args[0].strip(), args[1].strip()
            logging.debug('%*s#invoke %s %s %s', extractor.frame.depth, '', mod, fun, args[2:])
            # special handling of frame
            if len(args) == 2:
                # find parameters in frame whose title is the one of the original
                # template invocation
                templateTitle = fullyQualifiedTemplateTitle(mod)
            ...
            ret = sharp_invoke(mod, fun, params)
            logging.debug('%*s<#invoke %s %s %s', extractor.frame.depth, '', mod, fun, ret)
            return ret
...
def load_templates(file, output_file=None):
    ...
    for page_count, page_data in enumerate(pages_from(file)):
        id_, revid, title, ns, page = page_data
...
def pages_from(input_):
    ...
    for line in input_:
        ...
        if tag == 'page':
            ...
            if id_ != last_id and not redirect:
                yield (id_, revid, title, ns, page)
                last_id = id_
                ns = '0'
            id_ = None
            revid = None
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    if input_file == '-':
        ...
        input_ = fileinput.FileInput(input_file, openhook=fileinput.hook_compressed)
    ...
    if options.expand_templates:
        ...
        if template_file:
            if os.path.exists(template_file):
                ...
                load_templates(input_, template_file)
                input_.close()
                input_ = fileinput.FileInput(input_file, openhook=fileinput.hook_compressed)
    ...
    # Mapper process
    page_num = 0
    for page_data in pages_from(input_):
        id_, revid, title, ns, page = page_data
        if keepPage(ns, page):
            # slow down
            delay = 0
            ...
            job = (id_, revid, title, page, page_num)
...
def extract_process(opts, i, jobs_queue, output_queue):
    ...
    while True:
        ...
        if job:
            id_, revid, title, page, page_num = job
            try:
                ...
                text = ''
                logging.exception('Processing page: %s %s', id_, title)
...
def main():
    ...
    groupP.add_argument('--tokenize', action='store_true',
                        help='Produce text tokenized with spacy tokenizer')
    groupP.add_argument('--lowercase', action='store_true',
                        help='Lowercase the text before storing it in json')
    groupP.add_argument('--replace-numbers', action='store_true',
                        help='replace numbers with NUM token')
    groupP.add_argument('--lemmatize', action='store_true',
                        help='run spacys lemmatization')
    ...
    options.tokenize = args.tokenize
    options.lowercase = args.lowercase
    options.replace_numbers = args.replace_numbers
    options.lemmatize = args.lemmatize
    ...
    if args.article:
        ...
        for page_data in pages_from(file):
            id_, revid, title, ns, page = page_data
            Extractor(id_, revid, title, page).extract(sys.stdout)
        file.close()
        return
